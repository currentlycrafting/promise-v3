...
// ── Enhanced create: better wording, realistic deadline, reminder, progress ──
export async function formatNewPromiseFull(rawText, userDeadlineHint = '') {
  const prompt = `You are helping someone create a clear, achievable promise.

Raw input: ${rawText}
${userDeadlineHint ? `User's deadline hint: ${userDeadlineHint}` : ''}

Return ONLY these lines (one per line). Use the exact labels. If a value is not needed use "none" or leave empty.
Name: <short name, 2-6 words>
Type: <self|others|world>
Promise: I promise I will <one clear sentence - fix grammar and make it specific>
Deadline: <realistic duration: e.g. 7d, 30d, 2h, 1d 12h. If user's hint is unrealistic, suggest a realistic one.>
Reminder: <number> <minutes|hours|days> (e.g. 24 hours, 1 day, 30 minutes - when to remind before deadline; or "none")
Progress: <number or "none"> (e.g. 10 for "run 10 times", or none for simple complete/not)

Rules:
- Make the promise read clearly and be achievable
- Deadline must be a duration like 7d, 30d, 2h
- Reminder: only one of minutes, hours, days. Use "none" if no reminder needed
- Progress: integer for countable goals, or "none"
- Output plain text only, no markdown`

  const result = await TextGeneration.generate(prompt, { maxTokens: 180, temperature: 0.3 })
  return result.text
}

export function parseCreateFull(text) {
  const lines = (text || '').split('\n')
  let name = '', type = 'self', promise = '', deadline = '', reminder_value = null, reminder_unit = '', target_value = null
  for (const line of lines) {
    const l = line.trim()
    if (l.toLowerCase().startsWith('name:'))     name     = l.split(':').slice(1).join(':').trim()
    ...
    if (l.toLowerCase().startsWith('reminder:')) {
      const rest = l.split(':').slice(1).join(':').trim().toLowerCase()
      if (rest && rest !== 'none') {
        const match = rest.match(/(\d+)\s*(minutes?|hours?|days?)/)
        if (match) {
          reminder_value = parseInt(match[1], 10)
          reminder_unit = match[2].startsWith('min') ? 'minutes' : match[2].startsWith('hour') ? 'hours' : 'days'
        }
      }
    }
    if (l.toLowerCase().startsWith('progress:')) {
      const rest = l.split(':')[1].trim().toLowerCase()
      if (rest && rest !== 'none') {
        const n = parseInt(rest, 10)
        if (!Number.isNaN(n)) target_value = n
      }
    }
  }
  return { name, type, promise, deadline, reminder_value, reminder_unit, target_value }
}
